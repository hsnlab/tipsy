...
suites and a comprehensive background on data plane testing in the below
paper.
...
steady state, TIPSY also contains an additional set of "update" scenarios
that can be used to benchmark the SUT under dynamic updates initiated by
the control plane. The implementations that come with TIPSY pipelines
contain code to feed the updates, via the Controller module, into the SUT
...
|-----------------------------+----+----+--------+------------+----------+-----------+----------+-----+--------|
| Data Center GW (DCGW)       | x  | x  | VXLAN  | L2/L3      | L2/L3    | -         | -        | x   | OK     |
|-----------------------------+----+----+--------+------------+----------+-----------+----------+-----+--------|
| Mobile GW (MGW)             | x  | x  | GTP    | L2/L3/L4   | L2/L3    | x         | -        | -   | OK     |
|-----------------------------+----+----+--------+------------+----------+-----------+----------+-----+--------|
| Broadband Network GW  (BNG) | x  | x  | GRE    | L2/L3/L4   | L2/L3    | x         | x        | x   | TODO   |
|-----------------------------+----+----+--------+------------+----------+-----------+----------+-----+--------|
