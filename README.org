...
- Create the benchmark root directory as above.
...
- Place a main TIPSY configuration file =my_benchmark.json= into the root
...
- Generate the configuration for the individual test cases that make up
...
- Generate the sample traffic traces that will be fed to the SUT during
...
- Run the benchmark (this may take an even longer while).
...
#+END_SRC
  ...
  you must specify SSH passwords as clear text in the JSON config, etc., we
  finalizing this step is currently a main TODO item for TIPSY.

- Evaluate, and visualize: these features have not been implemented yet so
  you must do these manually by now.

- Clean up the benchmark directory by removing all temporary files (pcaps,
