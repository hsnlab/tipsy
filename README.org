...
parameters as controller by the =scale= setting:
...
the parameters specified as _lists_ in the config (=user= and =bst=)
...
If, on the other hand, =scale= is set to =outer=, then a separate test will
be run for each combination of the multi-valued settings (=user= and
=bst=), i.e., we get 4 test, first setting the (=user=, =bst=) tuple to
(1,5), then to (1,10), then to (2,5), and finally to (2,10).
...
directory, which will contain all files (configurations, traffic traces,
data plane configs, results, etc.) associated with the benchmark, and
creating the main JSON configuration for the benchmark.  Then, to actually
run the benchmark simply issue the necessary TIPSY commands executing the
benchmark driver =tipsy= in the benchmark root directory with different
