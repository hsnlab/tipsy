...
|------------------------------------------------------+-------------+------------+----------+-----------+----------+-----+--------|
| [[./doc/README.portfwd.org][Port forward (PORTfwd)]] | -           | -          | -        | -         | -        | -   | ovs, bess, ofdpa |
|------------------------------------------------------+-------------+------------+----------+-----------+----------+-----+--------|
| [[./doc/README.L2fwd.org][L2 forward (L2fwd)]]       | -           | L2         | -        | -         | -        | -   | ovs, bess, ofdpa |
|------------------------------------------------------+-------------+------------+----------+-----------+----------+-----+--------|
| [[./doc/README.L3fwd.org][L3 forward (L3fwd)]]       | -           | L2/L3      | L2/L3    | -         | -        | -   | ovs, bess, ofdpa |
|------------------------------------------------------+-------------+------------+----------+-----------+----------+-----+--------|
...
In continuation of the example, the following snippet defines the
environment of the mgw pipeline measurements.
...
The =environment= section is divided into two sections: parameters of the
SUT and parameters of the Tester. Further details are in the
[[./doc/README.config.org][detailed TIPSY configuration guide]].
...
2. Generate an initial main TIPSY configuration file into the root
   directory.
...
3. Edit the generated =main.json= config file according to your needs. The
   below configuration will set up the [[././doc/README.mgw.org][Mobile Gateway (MGW)]] (=mgw=)
   pipeline and benchmark both the OVS and the BESS backends, while scaling
   the number of users from 10 to 1000.
...
4. Generate the configuration for the individual test cases that make up
   ...
   a separate directory in =my_benchmark/measurements=.

   #+BEGIN_SRC sh
   tipsy config
   #+END_SRC

   This call will create the benchmark configuration from =main.json=,
...
6. Run the benchmark (this may take an even longer while).
...
7. Evaluate, and visualize: these features have not been implemented yet so
   you must do these manually by now.

8. Clean up the benchmark directory by removing all temporary files (pcaps,
   logs, etc.).
...
There is a TIPSY design document available [[./doc/README.vision.org][here]]
that summarizes a hypothetical TIPSY session once all missing pieces will
eventually have fallen into their place.
